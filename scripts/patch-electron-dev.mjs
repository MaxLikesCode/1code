// Patches the Electron.app bundle in node_modules to show "1Code Pro Max" name and icon in macOS dock during dev mode.
...
if (existsSync(plistPath)) {
  try {
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleName 1Code Pro Max" "${plistPath}"`)
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName 1Code Pro Max" "${plistPath}"`)
    console.log("[patch-electron-dev] Updated Info.plist: name -> 1Code Pro Max")
  } catch (e) {
